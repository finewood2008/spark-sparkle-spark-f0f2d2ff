...
interface ReviewCenterProps {
  /** Layout variant. 'page' = full two-column page; 'drawer' = stacked single-column suitable for narrow side sheet. */
  layout?: 'page' | 'drawer';
  /** Optional refresh trigger — bumping this number re-fetches history (used by drawer to refetch on open). */
  refreshKey?: number;
}

export default function ReviewCenter({ layout = 'page', refreshKey = 0 }: ReviewCenterProps) {
  ...
  const listPanel = (
    <div
      className={`rounded-2xl bg-card shadow-lg border border-border flex flex-col overflow-hidden ${
        isDrawer ? 'flex-shrink-0' : 'md:w-[40%]'
      }`}
    >
      <div className="flex items-center gap-1 p-2 border-b border-border overflow-x-auto">
        ...
        <button
          ...
          className="ml-auto shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          title="刷新"
        >
          <RefreshCw size={13} />
        </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <span className="text-sm">
              暂无{filter === 'all' ? '审核内容' : FILTERS.find(f => f.key === filter)?.label}
            </span>
          </div>
        ) : (
  ...
  const detailPanel = (
    <div
      className={`rounded-2xl bg-card shadow-lg border border-border flex flex-col overflow-hidden ${
        isDrawer ? 'flex-1 min-h-0' : 'md:w-[60%]'
      }`}
    >
      {!selected ? (
